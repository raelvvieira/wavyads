        ...
        {slides.map((slide, i) => {
          ...
          return (
            <GlassCard key={i}>
              ...
              {/* Estilo atual */}
              <div className="rounded-lg bg-accent/10 border border-accent/30 px-3 py-2 mb-2">
                <div className="text-[10px] uppercase tracking-wider text-white/40 mb-1">Estilo</div>
                <div className="flex items-center gap-2">
                  <span className="text-lg">{style.emoji}</span>
                  <div className="flex-1">
                    <div className="text-sm font-semibold text-white">{style.nome}</div>
                    <p className="text-[10px] text-white/60">{style.resumo}</p>
                  </div>
                </div>
              </div>
              ...
              {!img ? (
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="w-full btn-accent rounded-md py-2 text-xs font-semibold inline-flex items-center justify-center gap-2 mt-3"
                >
                  {isLoading ? (
                    <>
                ...
                /* Recriar com 3 estilos */
                <div className="space-y-2 mt-3">
                  <div className="text-[10px] uppercase tracking-wider text-white/40 font-semibold">Tentar outro estilo</div>
                  <div className="grid grid-cols-3 gap-1.5">
                    {WAVY_STYLES.map((altStyle) => (
                      <button
                        key={altStyle.id}
                        onClick={() => generateOne(i, altStyle.id)}
                        disabled={isLoading}
                        className={cn(
                          "rounded-md py-1.5 text-[10px] font-semibold inline-flex flex-col items-center justify-center gap-0.5 transition-all",
                          styleByIdx[i] === altStyle.id
                            ? "ring-2 ring-accent bg-accent/10"
                            : "glass hover:bg-white/10",
                        )}
                        title={altStyle.descricao_longa}
                      >
                        <span className="text-sm">{altStyle.emoji}</span>
                        <span className="leading-tight">{altStyle.nome.split(" ")[0]}</span>
                      </button>
                    ))}
                  </div>
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full glass rounded-md py-1.5 text-[10px] font-semibold hover:bg-white/10 inline-flex items-center justify-center gap-1.5"
                  >
